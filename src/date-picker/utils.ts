 ...
 * 日期选择器跳转公共函数
 */
export function onJumperClickSharedFn({
  trigger, partial, mode, month, year,
}: JumperClickSharedFn) {
...
/**
 * 时间选择切换操作公共函数(当enableTimePicker为true)
 */
export function onTimePickerChangeSharedFn(
  {
 ...
 * 预设按钮点击公共函数
 */
export function onPresetClickSharedFn({
  preset,
  context,
  onChange,
  formatRef,
 ...
 * 年份选择器改变公共函数
 */
export function onYearChangeSharedFn(
  nextVal: number,
  {
...
interface MonthChangeChangeSharedFn {
  partial: DateRangePickerPartial;
  enableTimePicker: boolean;
  activeIndex: Ref<number>;
  year: Ref<number[]>;
 ...
 * 月份选择器改变公共函数
 */
export function onMonthChangeChangeSharedFn(
  nextVal: number,
  {
    partial, enableTimePicker, activeIndex, year, month,
  }: MonthChangeChangeSharedFn,
) {
  let partialIndex = partial === 'start' ? 0 : 1;
  if (enableTimePicker) partialIndex = activeIndex.value;
